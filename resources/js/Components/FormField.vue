...
<template>
    <div class="grid gap-2">
        ...
        <div class="flex min-h-12 items-center rounded-lg border bg-surface" :class="error ? 'border-2 border-danger' : 'border-separator'">
            ...
            <input ref="input" :id="id" :name="id" :value="displayedValue" :required="required" :autocomplete="autocomplete || undefined" :placeholder="placeholder || undefined" :inputmode="variant === 'money' || variant === 'code' || variant === 'number' ? 'numeric' : (variant === 'phone' ? 'tel' : undefined)" :type="inputType" :aria-invalid="error ? 'true' : 'false'" :aria-describedby="describedBy" class="min-h-11 min-w-0 flex-1 rounded-lg bg-transparent px-3 text-base" @input="update" @blur="emit('blur')" />
