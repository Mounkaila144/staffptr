...
suivre ses `activation-instructions`. (opencode charge ces personas via `.opencode/agent/`.)

> ⚠️ Le bloc complet (personas inline, ~205 KB) a été réduit manuellement pour éviter de
> renvoyer ~51 000 tokens à chaque requête quand le cache de prompt n'est pas actif (GLM).
> Régénérer la version complète : `npx bmad-method install -f -i codex` (ou `npm run bmad:refresh`).
<!-- END: BMAD-AGENTS -->
